...
public:
    ...
    void findMax(int index, vector<string> &words, vector<int> &letterScores, unordered_map<char, int> letterCount, int currentVal)
    {
        if (index == words.size())
        {
        ...
        findMax(index + 1, words, letterScores, letterCount, currentVal);

        int wordScore = calculateScore(words[index], letterCount, letterScores);
        if (wordScore > 0)
        {
            findMax(index + 1, words, letterScores, letterCount, currentVal + wordScore);
        }
    }
    ...
    int maxScoreWords(vector<string> &words, vector<char> &letters, vector<int> &letterScores)
    {
        ...
        findMax(0, words, letterScores, letterCount, 0);
        return maxi;
    }
};